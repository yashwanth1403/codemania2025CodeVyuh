...
const ChallengesHub: React.FC = () => {
  ...
  useEffect(() => {
    ...
    challenges.forEach((challenge) => {
      const similarChallenges = challenges.filter(c => 
        c.id !== challenge.id && 
      ...
      if (similarChallenges.length > 0) {
        const randomSimilar = similarChallenges[Math.floor(Math.random() * similarChallenges.length)];
        if (!tempLines.some(line => 
      ...
      <main className="container mx-auto px-4 pt-32 pb-20">
        ...
        >
          ...
          <motion.div
            ...
            className="mb-10"
          >
            <h2 className="text-xl font-semibold mb-4 text-cosmic-light">Browse by Category</h2>
            <CategoryHoneycomb 
        ...
        <div>
          {filteredChallenges.length > 0 ? (
            ...
            `}>
              ...
              {viewMode === 'grid' && connectionLines.map((line, index) => {
                const fromChallenge = filteredChallenges.find(c => c.id === line.from);
                const toChallenge = filteredChallenges.find(c => c.id === line.to);
                
                // Only draw lines if both challenges are in the filtered results
                if (fromChallenge && toChallenge) {
                  return (
                    <div key={`line-${index}`} className="absolute inset-0 pointer-events-none">
