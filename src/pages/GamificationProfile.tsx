...
const leaderboardData = [
  { id: 'user-5', name: 'Jordan Lee', avatar: undefined, score: 8750, position: 1 },
  { id: 'user-2', name: 'Sam Harrison', avatar: undefined, score: 7320, position: 2 },
  { id: 'user-3', name: 'Taylor Swift', avatar: undefined, score: 6540, position: 3 },
  { id: 'user-4', name: 'Morgan James', avatar: undefined, score: 4200, position: 4 },
  { id: 'user-1', name: 'Alex Chen', avatar: undefined, score: 2850, position: 5, isCurrentUser: true },
  { id: 'user-6', name: 'Jamie Rodriguez', avatar: undefined, score: 2100, position: 6 },
  { id: 'user-7', name: 'Casey Kim', avatar: undefined, score: 1890, position: 7 },
  { id: 'user-8', name: 'Riley Johnson', avatar: undefined, score: 980, position: 8 },
];
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-300">Joined {userState.joinDate}</p>
                      </div>
                    </div>
                ...
                <CardContent className="space-y-6 pt-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-cosmic-dark/30 p-3 rounded-lg text-center">
                      ...
                      <p className="text-xl font-bold text-cosmic-accent">{userState.totalXP}</p>
                    </div>
                    <div className="bg-cosmic-dark/30 p-3 rounded-lg text-center">
                      <p className="text-sm text-gray-300">Challenges</p>
                      <p className="text-xl font-bold text-cosmic-accent">{userState.completedChallenges}</p>
                    </div>
                  </div>
                  
                  <div className="flex flex-wrap gap-2 mt-4">
                    {achievementsData.filter(a => a.earned).slice(0, 4).map((achievement) => (
                      <div key={achievement.id} className="scale-75 origin-top-left">
                        <Award className={`w-8 h-8 ${achievement.type === 'platinum' 
                          ? 'text-cyan-400' 
                    ...
                    {achievementsData.filter(a => a.earned).length > 4 && (
                      <div className="flex items-center justify-center w-8 h-8 bg-cosmic-dark/50 rounded-full">
                        <span className="text-xs">+{achievementsData.filter(a => a.earned).length - 4}</span>
                      </div>
                    )}
                  </div>
            ...
            <div className="w-full md:w-2/3">
              <Tabs defaultValue="achievements" className="w-full">
                <TabsList className="w-full bg-cosmic-dark/50">
                  <TabsTrigger value="achievements" className="flex-1 data-[state=active]:bg-cosmic-accent">
                    <Trophy className="w-4 h-4 mr-2" />
                    Achievements
                  </TabsTrigger>
                  <TabsTrigger value="leaderboard" className="flex-1 data-[state=active]:bg-cosmic-accent">
                    <Award className="w-4 h-4 mr-2" />
                    Leaderboard
                  </TabsTrigger>
                  <TabsTrigger value="milestones" className="flex-1 data-[state=active]:bg-cosmic-accent">
                  ...
                  <Card className="bg-cosmic-dark/30 border-cosmic-accent/20">
                    <CardHeader>
                      ...
                      <CardDescription className="text-gray-300">
                        Earned {achievementsData.filter(a => a.earned).length} of {achievementsData.length} achievements
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                ...
                <TabsContent value="milestones" className="pt-4">
                  <Card className="bg-cosmic-dark/30 border-cosmic-accent/20">
                    <CardHeader>
                      ...
                      <CardDescription className="text-gray-300">
                        Completed {milestonesData.filter(m => m.completed).length} of {milestonesData.length} milestones
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                        ...
                        <div className="space-y-8">
                          {milestonesData.map((milestone, index) => (
                            ...
                            >
                              ...
                              `}>
                                <h4 className={milestone.completed ? 'font-medium' : 'text-gray-400'}>
                                  {milestone.title}
                                </h4>
                              </div>
                            </div>
